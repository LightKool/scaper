...
class Config(object):
	...
	def __init__(self, values=None, extra_sources=()):
		if values is None:
			...
			default_config = get_data(__package__, 'default.conf')
	...
	def _getsources(self):
		sources = ['/etc/scraperd/scraperd.conf', r'c:\scraperd\scraperd.conf']
		sources += sorted(glob.glob('/etc/scraperd/conf.d/*'))
		sources += ['scraperd.conf']
		sources += [expanduser('~/.scraperd.conf')]
